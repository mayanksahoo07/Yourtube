...
import { Box, Center, Text, Avatar,AvatarBadge, Input,Button,Image, InputGroup, InputLeftElement,Spinner,SkeletonCircle, Skeleton, useColorMode } from "@chakra-ui/react"
...
const RobotComponent = ({username,mode}) => {
    return (
        <Box d="flex" justifyContent="center" alignItems="center" color="white" flexDirection="column" > 
          ...
          <Text as="h1" d="flex">
            Welcome, <Text color={mode === "light"  ?"#D2BB31" : "#FB5B78"} fontSize="1.1rem" fontWeight="bold" pl={1}>{username.slice(0,20)}!</Text>
          </Text>
...
function Messages() {
    ...
    const back = colorMode === "light" ? "rgb(255,255,255,0.4)" : "rgb(31, 34, 35)"
       ...
       var newArray = [...new Set(mergedArray)]
        ...
        allusers.map(async ({userId})=> {
            ...
            var dats = await getAllMessages({from : user, to : userId})
                ...
                if(dats?.data?.length > 0 && b.status !== "not found") {
          ...
          <Center w="100%" h="100%">
            <Box width="95%" height="100%" >
                    <Box width="40%" d="flex" pt={3} pb={5} >
                        <Text w="40%" as="h1" fontSize="1.7rem" color={colorMode === "light" ? "#1C1F20" : "rgb(255,255,255,0.90)"} fontFamily="sans-serif"   fontWeight="bold">Messages</Text>
                        {colorMode === "dark" && 
                        <InputGroup w="60%" >
                        ...
                        {colorMode === "light" && 
                        <InputGroup w="60%" >
                            <InputLeftElement children={<FiSearch size="1rem" color="#1C1F20" />} />
                            <Input _placeholder={{color:"black"}} color="black" _hover={{borderColor: "#5A5A5B"}} borderColor="#5A5A5B" focusBorderColor="#5A5A5B" onChange={filterFriends} value={search} w="100%" placeholder="Search for friend" />
                    ...
                    {isLoading ? <Center w="100%" h="70vh"><Spinner 
                                    ...
                                    emptyColor='grey'
                                /></Center> :
                    <Box d="flex" boxShadow={colorMode === "light" ? "6px 6px rgb(0,0,0,0.5)" : "6px 6px rgb(255,255,255,0.3)"} borderRadius="5px">
                        <Box width="40%" bg={colorMode==="light" ? "#DAD9D9":"rgb(89, 91, 93,0.55)"} height="100%" borderRadius="5px 0px 0px 5px">
                            <Box overflowY="auto" height="70vh">
                                    ...
                                    {!showSkeleton && <>
                                        { Array.from(friends?.reduce((m, t) => m.set(t.userId, t), new Map()).values()).map(({userId,ProfileAvatar,username,isOnline,message,sentAt}) => (
                                           ...
                                           <Box key={userId} bg={(isChatActive == true && selectedUserId == userId) ? back : ''} cursor="pointer" _hover={colorMode === "light" ? {bg : "rgb(255,255,255,0.4)"}:{bg: "rgb(31, 34, 35)"}} onClick={()=> {setIsChatActive(true);setSelectedUserId(userId);setSelectedUserAvatar(ProfileAvatar);setDisplayChatAvatar("");setDisplayChatUsername("")}}>
                                            <Center width="100%">
                                                <Box width="95%" d="flex" mt={3} mb={3}>
                                                    <Avatar w="2.5rem" h="2.5rem" src={ProfileAvatar} >
                                                        {isOnline ?<AvatarBadge borderColor="#2D2D2E" boxSize='0.85rem' bg='#55D64F' /> : <AvatarBadge borderColor="#2D2D2E" boxSize='0.85rem' bg='gray' />}
                                                    </Avatar>
                                                    <Box width="80%" ml={2}>
                                                        <Text as="h3" color={colorMode==="light" ? "#1C1F20" : "white"}>{username?.slice(0,20)}</Text>
                                                        <Text as="p" color={colorMode === "light" ? "#595B5D" : "rgb(255,255,255,0.5)"} fontSize="0.85rem" noOfLines={1}>{message.length > 0 ? message : "Hey there! let's chat"}</Text>
                                                    </Box>
                                ...
                                {skeletons.map(() => (
                                    <Box  >
                                              <Box  d="flex" mt={3} pl={5}>
                                                              <Box label="messages" cursor="pointer"  width="2.3rem" height="2.3rem" borderRadius="50%">
                        ...
                        <Box width="60%" bg="rgb(36, 38, 39,0.8)" height="100%" borderRadius="0px 5px 0px 0px" >
                            ...
                            <Box overflowY="auto" w="96%" height="60vh" ml={5} mr={5} socket={socket}>
                                {!isChatActive && <RobotComponent mode={colorMode} username={userName} />}
                                ...
                                <ScrollableFeed>
                                    ...
                                    <Box d="flex">
                                        <Link to={`/profile/${selectedUserId}`} >
                                            <Avatar w="2.2rem" h="2.2rem" src={displayChatAvatar} ml={2} mt={1} />
                                            </Link> 
                                            <Box pl={3}>
                                                <Text  fontSize="1.1rem" color="white">{displayChatUsername}</Text>
                                                <Text color="rgb(255,255,255,0.5)" fontSize="0.85rem">Hi there! let's chat</Text>
                                                </Box>
                                            </Box>}
                                    ...
                                    {chatSelected?.map((message) => (
                                        ...
                                        <Box w="auto"  d="flex" justifyContent="right" pr={5}>
                                            ...
                                            <Text color="white" bg="#4552FE" pl={5} ml={2} pt={2} pb={2} pr={5} borderRadius="10px 2px 10px 10px">  
                                                {message.message}
                                            </Text>
                                        ...
                                        <Box w="auto"  d="flex" >
                                            <Link to={`/profile/${selectedUserId}`} ><Avatar w="2rem" h="2rem" src={selectedUserAvatar} ml={1} mt={1} /></Link>
                                            <Text color="white" bg="rgb(18, 19, 20,0.2)" pl={5} ml={2} pt={2} pb={2} pr={5} borderRadius="2px 10px 10px 10px">  
                                                {message.message}
                                            </Text>
                                            <Center pl={5}>
                                                <Text color="rgb(255,255,255,0.5)" fontSize="0.8rem">{format(message.createdAt)}</Text>
                                    ...
                                    <Center width="100%" height="100%">
                                        <Box w="95%" h="90%" d="flex">
                                            <Box pr={5} pt={6} cursor="pointer" onClick={handleEmojiPickerhideShow} >
                                                ...
                                                <Input color="white" onKeyDown={keyDownHandler} p={0} isDisabled={!isChatActive} value={message} onChange={handleInputChange} height="100%" width="90%" placeholder="Send a message" border="none" _focus={{border : "none"}} />
                                            
                                            
                                            <Box pl={6} pt={1.5} d="flex">
                                            
                                           
                                                <Button color="white" isDisabled={!isChatActive} onClick={sendmessage} _hover={{bg : "rgb(69, 82, 254,0.8)"}} _active={{bg : "rgb(69, 82, 254,0.8)"}} bg="#4552FE" pb={1} height="90%" cursor="pointer" textAlign="center" rightIcon={<AiOutlineSend fill="white" />}>Send</Button>
                                            </Box>
                                        </Box>
                                    </Center>
