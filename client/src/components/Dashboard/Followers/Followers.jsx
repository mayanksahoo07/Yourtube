...
import { Box, Text, Center,Tabs, TabList, TabPanels, Tab, TabPanel, Input, InputGroup, InputLeftElement,Spinner,useColorMode} from "@chakra-ui/react"
    ...
    <Box width="100%" height="88%" bg={colorMode === "light" ? "#F2F2F2" : "#111315"} fontFamily="heading" >
      <Center w="100%" h="100%">
         <Box width="94%" height="100%" >
             <Box d="flex" borderBottom={colorMode === "light" ? "" : "1px solid rgb(96, 96, 96,0.6)"} width="100%">
                 <Box width="80%">
                     <Text width="100%" as="h1" fontSize="1.7rem" color={colorMode === "light" ? "#1C1F20" : "rgb(255,255,255,0.90)"} fontFamily="sans-serif" pt={3} pb={5}  fontWeight="bold">Followers</Text>
                 </Box>
                
                    {colorMode === "dark" && <InputGroup ml={28} pt={3} textAlign="right">
                    ...
                    {colorMode === "light" && <InputGroup ml={28} pt={3} textAlign="right">
                        <InputLeftElement pt={8} pl={2} ml={24} children={<FiSearch size="1.5rem" color="#1C1F20" />} />
                        <Input _hover={{borderColor: "#5A5A5B"}} borderColor="#5A5A5B" focusBorderColor="#5A5A5B" _placeholder={{color:"black" }} color="black" onChange={filter} value={search} ml={24} pl={12} width="80%"  height="3rem" placeholder="Search" />
                    </InputGroup>}
                ...
                {isLoading ? <Center w="100%" h="70vh"><Spinner 
                    ...
                    <Tabs isFitted >
                        {colorMode === "dark" &&<TabList width="100%">
                        ...
                        {colorMode=== "light" && <TabList borderRadius="5px" border="1px solid #111315">
                                <Tab fontWeight="600" _focus={{border:"none"}} _selected={{color : "white",bg : "#111315", border : "none"}}>All followers  <Text ml={16} fontWeight="bold">{allFollowers}</Text></Tab>
                                <Tab borderLeft="1px solid #111315" fontWeight="600" _focus={{border:"none"}} _selected={{color : "white",bg : "#111315", border : "none"}}>Followers online <Text ml={16} fontWeight="bold">{onlineFollowers}</Text></Tab>
                                <Tab borderLeft="1px solid #111315" fontWeight="600" _focus={{border:"none"}} _selected={{color : "white",bg : "#111315", border : "none"}}>Connect</Tab>
                            </TabList>}
                            ...
                            <TabPanel overflowY="auto" height="70vh">
                                ...
                                <Box d="grid" gridTemplateColumns="1fr 1fr" gap={6}>
                                    {allFollowersData.map(({username,ProfileAvatar,isVerified,followers,userId,follow,isOnline}) => (
                                       <FollowerComponent mode={colorMode} username={username} avatar={ProfileAvatar} isVerified={isVerified} 
                            ...
                            <TabPanel overflowY="auto" height="70vh">
                                ...
                                :
                                    <Box d="grid" gridTemplateColumns="1fr 1fr" gap={6}>
                                            {allFollowersData.map(({username,ProfileAvatar,isVerified,followers,userId,follow,isOnline}) => (
                                                <>
                                             {isOnline && <FollowerComponent mode={colorMode} username={username} avatar={ProfileAvatar} isVerified={isVerified} 
