...
import { Box, Text, Center, FormControl,useDisclosure,
  ...
  InputGroup, InputLeftElement, Button ,InputRightElement,Select, Avatar, Spinner, useToast, useColorMode} from "@chakra-ui/react"
...
const Copy = ({textValue,id}) => {
    return(
        <CopyToClipboard text={textValue} onCopy={() => setCopy(id)}>
            <Box>
                <Button height="2.8rem" width="6rem" border="1px solid rgb(255,255,255,0.4)" color={colorMode === "light" ? "#1C1F20" : "rgb(255,255,255,0.93)"} borderRadius="5px" fontSize="0.9rem">Copy</Button>
  ...
  const Copyied = ({textValue}) => {
    return(
        <CopyToClipboard text={textValue} onCopy={() => setCopy(0)}>
            <Box>
                <Button  height="2.8rem" width="6rem" border="1px solid rgb(255,255,255,0.4)" color={colorMode==="light" ? "#1C1F20" : "rgb(255,255,255,0.93)"} borderRadius="5px" fontSize="0.9rem">✔ Copied</Button>
         ...
         :
                    <Center mt={2} w="100%" h="90%" bgImage={`url('${universe}')`} bgRepeat="no-repeat" bgSize="cover" bgPosition="center" borderRadius="10px">
                        <Box textAlign="center" w="100%" h="100%" bgImage={`url('${astronaut}')`} bgRepeat="no-repeat" bgSize="contain" bgPosition="center">
                            ...
                            <Text color="white" fontSize="17rem" fontWeight="bold">4&nbsp;&nbsp;{error ? '4' : '5'}</Text>
         ...
         <Box width="94%" height="100%" d="flex" gap={5}>
            <Box width="70%" height="100%" >
                            ...
                            <Center width="100%" height="100%">
                                <Box width="90%" d="flex" height="80%">
                                    <Box width="80%">
                                        ...
                                        <Text as="h4" fontSize="1.1rem" color="rgb(255,255,255,0.90)" noOfLines={1}>{streamData?.title}</Text>
                                        ...
                                        <Box d="flex">
                                            ...
                                            <Box width="50%">
                                               <Text as="h4" fontSize="0.85rem" fontFamily="sans-serif" color="rgb(255,255,255,0.50)" pt={2}>Likes</Text>
                                                <Text as="h4" fontSize="0.9rem" color="rgb(255,255,255,0.90)">{numFormatter(streamData?.likes)}</Text> 
                                            </Box>
                ...
                <Box width="100%" height="60%" bg="#242627"  borderRadius="5px">
                    <Box width="100%" height="12%" bg="#1a1b1c" borderRadius="5px">
                                   ...
                                   <Text as="h3" color="rgb(255,255,255,0.50)" fontSize="1rem"  pt={4}>Server URL</Text>           
                                    <FormControl pt={2}>
                                        <InputGroup >
                                            ...
                                            <Input color="white" _placeholder={{color : "white"}} id='title' placeholder='Stream URL' height="3rem" value='rtmp://rtmp.livepeer.com/live/' />
                                    ...
                                    <FormControl pt={2}>
                                        <InputGroup >
                                            <InputLeftElement pt={2} pl={2} children={<BiLockOpen color="white" size="1.2rem" />}  />
                                            <Input color="white" _placeholder={{color : "white"}} id='title' placeholder='Key' height="3rem" value={streamData?.streamKey} />
                                    ...
                                    <FormControl pt={2}>
                                        <InputGroup >
                                            ...
                                            <Input color="white" _placeholder={{color : "white"}} id='title' placeholder='Playback URL' height="3rem" value={`https://account.yourtube.io/live/${streamData?.streamUrl}`} />
                                            <InputRightElement pt={2} pr={12} children={copy === 3 ? <Copyied textValue={`https://account.yourtube.io/live/${streamData?.streamUrl}`}/>  : <Copy id={3} textValue={`https://account.yourtube.io/live/${streamData?.streamUrl}`}/>} />
                                        </InputGroup>
                                    </FormControl>
                             </Box>
            ...
            <Box width="30%" height="100%"  borderRadius="5px" bg={colorMode==="light" ? "#111315" : ""}>
                    <Box height="8%" width="100%" position="sticky" bg="#242627" borderRadius="5px 5px 0px 0px">
                        <Center width="100%" height="100%" pb={1.5} color="white"><Text>Stream Chat</Text></Center>                    
                    </Box>
                          ...
                          <Box width="100%" d="grid" gridTemplateRows="4fr 4fr 4fr 4fr" gap={5} pt={5} pb={5}>
                            {chatSelected?.map((message) => (
                                <Box w="auto"  d="flex">
                                    ...
                                    <Box>
                                      <Box d="flex" pl={3}>
                                        <Text as="h4" color="#FBFBFF">{message?.creator?.username?.slice(0,12)}</Text>
                    ...
                    <Box height="15%" width="100%" bg="#242627" borderRadius="0px 0px 5px 5px">
                        <Center width="100%">
                            <Box width="90%" >
                            <Box d="flex" pt={1.5}>
                                    ...
                                    <Box pl={5}>
                                        <Box>
                                          <Text as="h3" fontSize="0.9rem" color={colorMode === "light" ? "#FBFBFF" : "#FFD600"} fontWeight="600" cursor="pointer">{userData?.username}</Text>
                                         
                                        </Box>
                                      <InputGroup pt={2}>
                                        <Input _placeholder={{color: "white"}} color="white" onKeyDown={keyDownHandler}  value={message} onChange={handleInputChange} placeholder='Say something...' />
          ...
          <ModalOverlay />
            ...
            <ModalContent bg="#F7F7FC">
              ...
              <Center width="100%"  height="69vh" mt={5}>
                  <Box pl={4} width="100%" pb={5} bg={colorMode === "light" ? "#DAD9D9" : "#242627"} d="flex"  height="100%" overflowY="auto">
                    <Box width="93%">
                      <Text as="h1" color={colorMode === "light" ? "#1C1F20" : "rgb(255,255,255,0.95)"} fontWeight="bold" fontSize="1.6rem" pb={4}>Details</Text>
                      ...
                      <Text as="h3" color={colorMode === "light" ? "#1C1F20" : "rgb(255,255,255,0.95)"} fontSize="1.1rem" fontFamily="sans-serif"  fontWeight="bold" pt={5}>Category</Text>           
                      <Text as="p" fontSize="0.9rem" color={colorMode==="light" ? "#595B5D": "rgb(255,255,255,0.50)"} pt={2}>Add your stream to a category so viewrs can find it more easily</Text>
                      <FormControl pt={3}>
                ...
                <Box width="100%" borderTop="1px solid rgb(96, 96, 96,0.6)" height="4rem" position="sticky" textAlign="end">
                    ...
                    <Button color="white" onClick={() => {onClose();EditStream()}} fontSize="1.15rem" bg="#3ea6ff" mr={5} pb={1} height="3rem" width="7rem" mt={3} _hover={{bg : 'rgb(62, 166, 255, 0.85)'}} 
